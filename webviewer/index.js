...
function initFromParameters() {
  const params = new URL(window.location.href).searchParams;  // 获取参数
  ...
  if (stepMult) {
    // 解析一个字符串并返回指定基数(此处表示待解析的字符串中有十进制数)的十进制整数
  ...
  // 如果给了downscale参数则lowResFactor=downscale，否则lowResFactor=1
  // 此处，lowResFactor=1
  let lowResFactor = parseInt(params.get('downscale') || 1, 10);

  // 如果给了size参数，则根据size参数重新设置frameBufferWidth和frameBufferHeight的值
  if (size) {
    const match = size.match(/([\d]+),([\d]+)/);
    frameBufferWidth = parseInt(match[1], 10);
    frameBufferHeight = parseInt(match[2], 10);
  // 如果给了quality参数，则根据quality参数重新设置frameBufferWidth和frameBufferHeight的值
  // 根据quality参数来决定画面的的大小
  } else if (quality) {
  ...
  if (!params.get('downscale') && quality) {
    ...
    // 使用一个while循环来不断增加lowResFactor，直到满足阈值条件
  ...
  // 创建一个由标签名称div指定的HTML元素,并将该元素的类设置为view，并且将该元素返回
  ...
  // 获得id为viewspacecontainer的元素
  const viewSpaceContainer = document.getElementById('viewspacecontainer');
  // 让viewspacecontainer显示，显示类型为inline-block行内块元素
  ...
  gRenderer = new THREE.WebGLRenderer({
    canvas: canvas,  // 渲染器绘制输出的画布,不给这个参数会自己创建，可以通过renderer.domElement访问画布
    context: gl,
  ...
  gCamera = new THREE.PerspectiveCamera(
      ...
      nearPlane, 100.0);
  gCamera.fov = vfovy;  // 摄像机视锥体垂直视野角度
